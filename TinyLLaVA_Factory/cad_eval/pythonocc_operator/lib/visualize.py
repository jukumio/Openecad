...
def create_edge_3d(curve: CurveBase, sketch_plane: CoordSystem):
    ...
    if isinstance(curve, Line) or type(curve).__name__ == Line.__name__:
    ...
    elif isinstance(curve, Circle) or type(curve).__name__ == Circle.__name__:
    ...
    # elif isinstance(curve, Arc):
    elif isinstance(curve, Arc) or type(curve).__name__ == Arc.__name__:
        ...
        raise NotImplementedError(type(curve), Circle, Line, Arc)
